...
def get_dataset(root, name):
    if name == 'mnist':
        ...
        # transform：图像类型的转换
        train_dataset = tv.datasets.MNIST(root, train=True, download=True, transform=tv.transforms.ToTensor())
        eval_dataset = tv.datasets.MNIST(root, train=False, transform=tv.transforms.ToTensor())
    elif name == 'cifar':
        # 设置两个转换格式
        ...
        eval_dataset = tv.datasets.CIFAR10(root, train=False, transform=transform_test)
